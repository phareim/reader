...
- `utils/decay.ts` — half-life aging for the deck (2026-08-10): `decayAge(publishedAt, halfLifeHours, now)` (age in half-lives; missing/invalid dates read as 0 — never fade), `hasFaded` (past `FADE_HORIZON` = 3 half-lives), `halfLifeLabel` / `nextHalfLife` (the Sources "Pace" cycle: 12h → 1d → 3d → 7d → 30d), `softCount(n)` (the deck header's flexible count — precise to `COUNT_CAP` = 40, then "40+"), `DECAY` constants (default half-life 72h). **The SQL in `server/api/articles/index.get.ts` mirrors `decayAge`/`hasFaded` — change them in lockstep.**
