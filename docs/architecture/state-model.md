...
**Interest score** (`Article.interest`, migration `021-article-interest.sql`, 2026-09-17): REAL 0..2, NULL = unscored. Filled by `POST /api/internal/score-interest` via TypeSafe Jev (a hosted decision model — see "Interest score (TypeSafe Jev)" in [`../../server/CLAUDE.md`](../../server/CLAUDE.md) for the eval numbers: AUC 0.80 overall, 0.70 within the same feed). Used only as a gentle boost to the deck's decay half-life (`0.75 + 0.25 * COALESCE(interest, 1)`, unscored = neutral 1.0) — it reorders decay-mode queries, it never fades or filters an article.
