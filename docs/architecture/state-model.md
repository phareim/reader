...
**Half-life ("the pace", migration `019-feed-half-life.sql`, 2026-08-10)**: `Feed.half_life_hours` (REAL, NULL = default 720h/30d, `-1` = the ∞ pace: never fades, orders by the default) drives the deck's decay mode — unread queries with `decay=true` order by age-in-half-lives (a slow blog's week-old essay ranks with today's news) and **fade** rss articles older than 3 half-lives. Fading is a filter, not a verb: nothing is marked read, the article stays reachable via search and mark-all-read still clears it. This deliberately makes "how many unread" a definitional question — the deck counts what is *currently flowing*, not the backlog (and the header shows "40+" past `DECAY.COUNT_CAP`). Pure logic in `utils/decay.ts`; the Sources "Pace" row button cycles presets via `PATCH /api/feeds/:id`. Found/manual feeds never fade.
...
**Interest score** (`Article.interest`, migration `021-article-interest.sql`, 2026-09-17): REAL 0..2, NULL = unscored. Filled by `POST /api/internal/score-interest` via TypeSafe Jev (a hosted decision model — see "Interest score (TypeSafe Jev)" in [`../../server/AGENTS.md`](../../server/AGENTS.md) for the eval numbers: AUC 0.80 overall, 0.70 within the same feed). Used only as a gentle boost to the deck's decay half-life (`0.75 + 0.25 * COALESCE(interest, 1)`, unscored = neutral 1.0) — it reorders decay-mode queries, it never fades or filters an article.
...
`database/migrations/`, applied by CI before every deploy — see the root `AGENTS.md`
under "Deployment".
