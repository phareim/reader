...
Copy `.env.example` to `.env.local`, set Supabase credentials, and never commit secrets. Run `npm install` once per Node 22.x environment so `postinstall` can prepare Nuxt. Before deploying, ensure Supabase env vars are set for the target environment. If handling RSS HTML, rely on the existing DOMPurify utilities in `server/utils/` rather than crafting new sanitizers.
