...
Nuxt 3 pages live under `pages/`, while reusable UI lives in `components/` (grouped by feature) and global state helpers sit in `composables/`. Server endpoints are colocated in `server/api/<domain>/<action>.ts`, sharing helpers in `server/utils/`. Database models live in `prisma/schema.prisma` and migrations within `prisma/migrations/`. Assets such as favicons or mock data belong to `public/` and `assets/`, and TypeScript helpers are in `types/` and `utils/`. Jest specs mirror features inside `__tests__/components` and `__tests__/utils`.
...
- `npm run prisma:migrate` runs `prisma migrate dev`; use `npx prisma migrate deploy` before deploying to Neon/Postgres.
...
Use 2-space indentation and TypeScript everywhere Nuxt allows (`<script setup lang="ts">`). Vue components are PascalCase (`components/layout/PageHeader.vue`), composables use the `useX` prefix, and server handlers follow HTTP verb suffices (`feeds/[id].post.ts`). Favor Tailwind utility classes from `tailwind.config.js` instead of ad-hoc CSS. Keep Prisma schema changes formatted via `npx prisma format`, and prefer small, focused files over large multi-purpose modules.
...
Recent history favors short, descriptive subjects such as `performance improvements` or `Bump glob from 10.4.5 to 10.5.0`; follow that present-tense style and include scope if needed (`feeds: debounce refresh button`). Reference related issues in the body and keep commits logically grouped (e.g., schema change + migration in one commit). PRs should summarize behavior, list verification steps (`npm run test`, screenshots for UI tweaks), and mention any migrations or env changes. Tag reviewers on cross-cutting changes (auth, sync engine, Prisma).

## Security & Configuration Tips
Copy `.env.example` to `.env.local`, set Postgres, Auth.js, and Google credentials, and never commit secrets. Run `npm install` once per Node 22.x environment so `postinstall` can generate Prisma clients. Before deploying, ensure `DATABASE_URL` points to the managed database and run `prisma migrate deploy`. If handling RSS HTML, rely on the existing DOMPurify utilities in `server/utils/` rather than crafting new sanitizers.
