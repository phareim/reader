...
- `__tests__/components/SourcesPage.test.ts` — the `/sources` room's feed rows: one-line row (title link, count hidden at zero), verbs folded behind the `···` toggle (one row open at a time, aria-expanded), Mark read call-through, Delete clears the open state
