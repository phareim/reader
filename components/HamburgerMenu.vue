        ...
        <div class="p-2 space-y-3">
          ...
          <div class="space-y-3">
            ...
            <div v-else class="space-y-1">
              <!-- Tag Folders -->
              <div v-for="tag in allTags" :key="tag" class="space-y-0">
                ...
                <Transition name="expand">
                  <div v-if="openTags.has(tag)" class="ml-4 space-y-0">
                    <div
                      v-for="feed in feedsByTag[tag]"
                      :key="feed.id"
                      class="flex items-center gap-1 relative"
                    >
                      <button
                        @click="selectFeed(feed.id, tag)"
                      ...
                      >
                        ...
                        <span v-if="feed.unreadCount > 0" class="flex-shrink-0 text-xs bg-blue-500 dark:bg-blue-600 text-white px-2 py-0.5 rounded-full">
                          {{ feed.unreadCount }}
                        </span>
                      </button>
              ...
              <div v-if="feedsByTag['__inbox__'] && feedsByTag['__inbox__'].length > 0" class="space-y-0 border-t border-gray-200 dark:border-gray-700 pt-2 mt-2">
                <div
                  class="w-full flex items-center gap-2 px-2 py-1.5 text-sm font-medium rounded transition-colors group"
                ...
                >
                  <button
                    @click.stop="toggleTagFolderOnly('__inbox__')"
                    class="p-0.5 hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition-colors"
                  >
                    <svg
                      class="w-4 h-4 transition-transform"
                      :class="{ 'rotate-90': openTags.has('__inbox__') }"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                <Transition name="expand">
                  <div v-if="openTags.has('__inbox__')" class="ml-4 space-y-0">
                    <div
                      v-for="feed in feedsByTag['__inbox__']"
                      :key="feed.id"
                      class="flex items-center gap-1 relative"
                    >
                      <button
                        @click="selectFeed(feed.id, '__inbox__')"
                        class="flex-1 min-w-0 text-left px-3 py-1.5 text-sm rounded transition-colors flex items-center gap-2"
                        :class="selectedFeedId === feed.id ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-900 dark:text-blue-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
                      >
                        <img v-if="feed.faviconUrl" :src="feed.faviconUrl" alt="" class="w-4 h-4 flex-shrink-0" />
