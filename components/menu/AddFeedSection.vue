<template>
  <div class="space-y-2">
    <input v-model="newFeedUrl" type="url" placeholder="Enter URL (feed or website)"
      class="w-full px-3 py-2 text-base border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
    ...
    <Transition name="fade-scale">
      <div v-if="newFeedUrl.trim() !== ''" class="flex gap-2 items-center justify-between w-full overflow-hidden">
        <button @click="handleDiscoverFeeds" :disabled="!newFeedUrl.trim() || discovering"
          class="flex-1 px-3 py-2 text-base bg-purple-500 dark:bg-purple-600 text-white rounded-lg hover:bg-purple-600 dark:hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          {{ discovering ? 'Discovering...' : 'Discover' }}
        </button>
        <button @click="handleAddFeed" :disabled="!newFeedUrl.trim() || loading"
          class="flex-1 px-3 py-2 text-base bg-blue-500 dark:bg-blue-600 text-white rounded-lg hover:bg-blue-600 dark:hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
    ...
    <div v-if="discoveredFeeds.length > 0"
      ...
      <h4 class="text-base font-semibold text-purple-900 dark:text-purple-300">Discovered Feeds:</h4>
      <div class="space-y-1">
        <button v-for="(feed, index) in discoveredFeeds" :key="index" @click="addDiscoveredFeed(feed.url)"
          class="w-full text-left px-3 py-2 text-base bg-white dark:bg-zinc-800 hover:bg-purple-100 dark:hover:bg-purple-900/30 rounded border border-purple-200 dark:border-purple-700 transition-colors">
